...
class deepseek_vl_chat():
  ...
  def test_cot(self, image_path, json_path):
    ...
    with tqdm(total=len(data)) as pbar:
      for d in data:
        ...
        prompt = self.get_prompt(cot_prompt, text)
        image_path_ = f"{image_path}/{image_id}.jpg"
        gen_text = self.deepseek_vl_generate_single(image_path_, prompt)
        ...
        output_data = (image_id, text, label, gen_text, predict_label)
        json_ = self.get_json(output_data)
        output_json.append(json_)

        pbar.update(1)
    output_json = json.dumps(output_json)

    output_file = open('./saved_documents/deepseek_vl_chat/output_cot.json', 'w', encoding='utf-8')
    output_file.write(output_json)
    output_file.close()

    return (true_labels, predict_labels)
  
  def test_ape_cot(self, image_path, json_path):
    data = self._get_dataloader(json_path)
    ...
    with tqdm(total=len(data)) as pbar:
      for d in data:
        (image_id, text, label) = d
        prompt = self.get_prompt(ape_cot_prompt, text)
        image_path_ = f"{image_path}/{image_id}.jpg"
        gen_text = self.deepseek_vl_generate_single(image_path_, prompt)
    ...
    output_file = open('./saved_documents/deepseek_vl_chat/output_ape_cot.json', 'w', encoding='utf-8')
    output_file.write(output_json)
    output_file.close()

    return (true_labels, predict_labels)
  
  def test_ps(self, image_path, json_path):
    data = self._get_dataloader(json_path)
    ...
    with tqdm(total=len(data)) as pbar:
      for d in data:
        (image_id, text, label) = d
        prompt = self.get_prompt(ps_prompt, text)
        image_path_ = f"{image_path}/{image_id}.jpg"
        gen_text = self.deepseek_vl_generate_single(image_path_, prompt)
    ...
    output_file = open('./saved_documents/deepseek_vl_chat/output_ps.json', 'w', encoding='utf-8')
    output_file.write(output_json)
    output_file.close()

    return (true_labels, predict_labels)
  
  def test_gkp(self, image_path, json_path):
    data = self._get_dataloader(json_path)
    ...
    with tqdm(total=len(data)) as pbar:
      for d in data:
        (image_id, text, label) = d
        image_path_ = f"{image_path}/{image_id}.jpg"
        prompt = self.get_prompt(gkp_prompt_1, text)
        knowledge = self.deepseek_vl_generate_single(image_path_, prompt)
        knowledge = str(knowledge)
        prompt = self.get_prompt(gkp_prompt_2, text)
        prompt = prompt.replace('<knowledge>', knowledge)
        gen_text = self.deepseek_vl_generate_single(image_path_, prompt)
        gen_text = 'Knowledge: ' + knowledge + '\nGen_text: ' + gen_text
    ...
    output_file = open('./saved_documents/deepseek_vl_chat/output_gkp.json', 'w', encoding='utf-8')
    output_file.write(output_json)
    output_file.close()
